...
const User = () => {
    ...
    const [formCounter, setFormCounter] = useState<number | null>(null); 

    useEffect(() => {
        if (typeof window !== "undefined") {
            const counter = localStorage.getItem('form_counter');
            const currentCounter = counter ? parseInt(counter, 10) : 1;
            setFormCounter(currentCounter); // Set the form counter state
        }
    }, []);
    ...
    const createNewForm = () => {
        if (formCounter === null) return; // Ensure formCounter is not null
        ...
        <div className="min-h-screen bg-gray-100">
            ...
            <section className="px-6 md:px-20">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mt-4">
                    ...
                    { title: "Party Invite", thumbnail: "/image-4.png" }].map((template, index) => (
                        <button type="button" onClick={createNewForm} key={template.title}>
                            <div className="border hover:shadow-md hover:border-purple-600 transition-shadow cursor-pointer bg-white p-3 rounded-md">
                                <img
            ...
            <section className="mt-10 px-6 md:px-20">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                    {[{ title: "Untitled Form", thumbnail: "/image-5.png", date: "Opened Nov 17, 2024" },
                    { title: "Job Application", thumbnail: "/image-1.png", date: "Opened Nov 17, 2024" }].map((form, index) => (
                        <div key={index} className="border hover:shadow-md hover:border-purple-600 transition-shadow cursor-pointer bg-white p-3 rounded-md">
                            <img
                                src={form.thumbnail}
                                alt={form.title}
                                className="w-full h-32 object-cover rounded-md"
